...
class Chexpert(RNGDataFlow):
    ...
    def __init__(self, folder, train_or_valid='train', channel=1, resize=None, shuffle=False):
        ...
        self.small = True if "small" in self.folder
        ...
        self.df['No Finding'][(self.df['No Finding'] != 1.0) & (self.df['No Finding'] != 0.0)] = 0.5
        self.df['Enlarged Cardiomediastinum'][(self.df['Enlarged Cardiomediastinum'] != 1.0) & (self.df['Enlarged Cardiomediastinum'] != 0.0)] = 0.5
        self.df['Cardiomegaly'][(self.df['Cardiomegaly'] != 1.0) & (self.df['Cardiomegaly'] != 0.0)] = 0.5
        self.df['Lung Opacity'][(self.df['Lung Opacity'] != 1.0) & (self.df['Lung Opacity'] != 0.0)] = 0.5
        self.df['Lung Lesion'][(self.df['Lung Lesion'] != 1.0) & (self.df['Lung Lesion'] != 0.0)] = 0.5
        self.df['Edema'][(self.df['Edema'] != 1.0) & (self.df['Edema'] != 0.0)] = 0.5
        self.df['Consolidation'][(self.df['Consolidation'] != 1.0) & (self.df['Consolidation'] != 0.0)] = 0.5
        self.df['Pneumonia'][(self.df['Pneumonia'] != 1.0) & (self.df['Pneumonia'] != 0.0)] = 0.5
        self.df['Atelectasis'][(self.df['Atelectasis'] != 1.0) & (self.df['Atelectasis'] != 0.0)] = 0.5
        self.df['Pneumothorax'][(self.df['Pneumothorax'] != 1.0) & (self.df['Pneumothorax'] != 0.0)] = 0.5
        self.df['Pleural Effusion'][(self.df['Pleural Effusion'] != 1.0) & (self.df['Pleural Effusion'] != 0.0)] = 0.5
        self.df['Pleural Other'][(self.df['Pleural Other'] != 1.0) & (self.df['Pleural Other'] != 0.0)] = 0.5
        self.df['Fracture'][(self.df['Fracture'] != 1.0) & (self.df['Fracture'] != 0.0)] = 0.5
        self.df['Support Devices'][(self.df['Support Devices'] != 1.0) & (self.df['Support Devices'] != 0.0)] = 0.5
    ...
    def __iter__(self):
        ...
        for idx in indices:
            image_path = os.path.join(self.folder, self.ds.iloc[idx]['Path'])
            print(image_path)
...
if __name__ == '__main__':
    ds = Chexpert(folder='/u01/CheXpert-v1.0/', 'train')
    ds.reset_state()
    for _ in ds:
...
# # ! ln -s /u01/data/CheXpert-v1.0-small chexpert
# print(tfds.list_builders()) \
# # print(info)
# data, info = tfds.load("chexpert", with_info=True)
